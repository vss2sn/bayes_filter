...
int main() {
  ...
  Eigen::VectorXd min_state((Eigen::VectorXd(2) << -10, -10).finished());
  Eigen::VectorXd max_state((Eigen::VectorXd(2) << 10, 10).finished());
  Particle pf(P, Q, R, processFunction, outputFunction, min_state, max_state, n_particles);
  ...
  // pf.SetInitialState((Eigen::VectorXd(2) << 0.0, 0.0).finished());
  ...
  for (int i = 1; i < N; ++i) {
    ...
    // Eigen::VectorXd q = A * (Eigen::VectorXd(2) << true_pos[i - 1], true_vel[i - 1]).finished() + B * (Eigen::VectorXd(1) << true_acc[i]).finished();
    ...
    // Predict and update
    pf.Predict((Eigen::VectorXd(1) << believed_acc[i]).finished());
    pf.Update((Eigen::VectorXd(1) << measured_pos[i][0]).finished());

    // Get the estimated state
    estimated_pos[i] = pf.GetState()[0];
